...
)
{
    ...
    {
        Info<< "Selecting solidChemistryModel " << userModel + gasThermoName + modelType << "\n \n modelType: " << modelType 
            << endl;
    }
    ...
    return autoPtr<solidChemistryModel>
        (cstrIter()(mesh, obj, ODEModelName, thermoTypeName));
}
*/
//************************************************************************** //
...
)
{
    ...
    {
        Info<< "Selecting solidChemistryModel " << userModel << "\n \n modelType: " << modelType 
            << endl;
    }

    fvMeshGasPhaseGasesConstructorTable::iterator cstrIter =
        fvMeshGasPhaseGasesConstructorTablePtr_->find(modelType);

    if (cstrIter == fvMeshGasPhaseGasesConstructorTablePtr_->end())
    {
        if (debug)
        {
            FatalErrorIn("solidChemistryModel::New(const mesh&)")
                ...
                << modelType << nl << nl
                << "Valid solidChemistryModel types are:" << nl
                << fvMeshGasPhaseGasesConstructorTablePtr_->sortedToc() << nl
                << exit(FatalError);
        }
        else
        {
            wordList models = fvMeshGasPhaseGasesConstructorTablePtr_->sortedToc();
 		
                Info << "Unknown solidChemistryModel type "
                << userModel << nl << nl
                << "Valid solidChemistryModel types are:" << nl
                << models << nl;
    ...
    return autoPtr<solidChemistryModel>
        (cstrIter()(mesh, obj, ODEModelName, thermoTypeName, gasPhaseGases)); //wywoluje ze wszystkich znanych konstruktorow odpowieniej nazwy.
}
